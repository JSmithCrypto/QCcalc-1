parameterSettings = [25 10 10 20; 50 10 20 20; 75 10 30 20; 100 10 40 20];

for thisLoop = 1:size(parameterSettings,1)   

close all;

%% INPUT PARAMETERS
% set these values for yourself
par.yearly_increaseInQubits = parameterSettings(thisLoop,1); % in percent (0-100) -- note: 100% improvement is doubling the # qubits every year.
par.yearly_errorRateImprovement = parameterSettings(thisLoop,2); % in percent (0-100) -- note: 50% = cutting the error rate in half every year.
par.yearly_algorithmicImprovement = parameterSettings(thisLoop,3); % in percent(0-100) -- note: 50% = cutting the # of LOGICAL qubits required in half every year
par.parameter_uncertainty = parameterSettings(thisLoop,4); % percent uncertainty about the above parameters
...
% Options:
opt.nSamples = 50000; % higher count --> longer runtime, less-noisy estimates
opt.nYears = 50; % should be large enough to contain the majority of the resulting probability distribution distribution
...
opt.plotColor = [190 0 0]; % default is [190 0 0], a dark red color. Can be any RGB triplet.
...
for sample = 1:opt.nSamples
    ...
    if mod(sample,100) == 0, clc; disp(['run ' num2str(thisLoop) '/' num2str(size(parameterSettings,1)) ' | ' ...
            num2str(round((sample/opt.nSamples)*100)) '% done sampling']); end
...
%       maximize compatibility with Octave
    ...
    set(h,'FaceColor',opt.plotColor/255)
    xlim([0 opt.nYears]);
...
if opt.wannaPlot
    figure;
    plot(cumulativeProb,'LineWidth',3,'Color',opt.plotColor/255);
